...
// This prevents a top-level error in AuthVerifyPage from crashing the whole script immediately
const mountNode = document.getElementById('auth-verify-page');

if (!mountNode) {
  console.error('Failed to find mount node #auth-verify-page');
  document.body.innerHTML = '<div style="color: red; padding: 20px;">Error: Mount node not found.</div>';
} else {
  // Simple error boundary wrapper
  const renderApp = async () => {
    try {
      const { default: AuthVerifyPage } = await import('./islands/pages/AuthVerifyPage/AuthVerifyPage.jsx');
      createRoot(mountNode).render(<AuthVerifyPage />);
    } catch (error) {
      console.error('Failed to render AuthVerifyPage:', error);
      mountNode.innerHTML = `
        <div style="font-family: sans-serif; color: #333; padding: 40px; text-align: center; max-width: 600px; margin: 0 auto; margin-top: 50px;">
          ...
          <p style="font-size: 1.1em; margin-bottom: 30px;">We couldn't load the verification page.</p>
