 ...
 *
 * Creates guest payment records for leases based on calculated payment schedules.
 * Replaces Bubble's CORE-create-guest-payment-records-recursive-javascript workflow.
 *
 * Actions:
 ...
 * }
 *
 * KEY DIFFERENCES FROM HOST PAYMENT RECORDS:
 * - First payment: 3 days BEFORE move-in (vs 2 days AFTER for host)
 * - Updates 'Payment Records Guest-SL' field on lease
 * - Updates 'Total Rent' field on lease (vs 'Total Compensation' for host)
 * - Total Rent excludes damage deposit (it's refundable)
 */
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    console.log(`[guest-payment-records] Action: ${typedAction}`);
    ...
    console.error(`[guest-payment-records] Error:`, error);
