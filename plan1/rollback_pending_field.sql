-- Rollback Migration: remove_pending_field_from_all_tables
-- Description: Removes the 'pending' column from all non-OS tables
-- Use this if you need to revert the add_pending_field migration
...
BEGIN
    ...
    LOOP
        IF EXISTS (
            ...
            EXECUTE format('ALTER TABLE public.%I DROP COLUMN pending', tbl);
            RAISE NOTICE 'Dropped pending column from %', tbl;
        END IF;
    END LOOP;
END $$;
