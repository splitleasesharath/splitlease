...
```
Read the instructions in refactor/verify_barrel_removal.md and verify the fixes are complete
```
...
```bash
uv run refactor/detect_barrel_files.py app/src --min-reexports 1
```
...
uv run refactor/adw_visual_parity.py --pages "/search,/view-listing"
```
...
cd .. && uv run refactor/detect_barrel_files.py app/src --min-reexports 2 2>/dev/null | grep -E "Barrel files found|High severity|Total star"
