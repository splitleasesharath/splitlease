...
def validate_worktree(adw_id: str, state: ADWState) -> Tuple[bool, Optional[str]]:
    ...
    # Normalize paths for comparison - git uses forward slashes on all platforms
    worktree_path_normalized = worktree_path.replace("\\", "/")
    stdout_normalized = result.stdout.replace("\\", "/")

    if worktree_path_normalized not in stdout_normalized:
        return False, f"Worktree not registered with git. Looking for: {worktree_path_normalized}"
