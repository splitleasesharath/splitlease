...
def extract_adw_info(text: str, temp_adw_id: str) -> ADWExtractionResult:
    ...
    try:
        ...
        # Handle both success and failure cases - try to parse output either way
        # This handles Claude Code v2.0.30+ where slash commands return raw text
        # but execute_template returns success=False due to missing JSONL format
        output_to_parse = response.output

        # If response failed, try reading raw output file as fallback
        if not response.success:
            print(f"Classification returned error: {response.output}")
            # Try to read the raw output file directly
            output_file = os.path.join(
                "agents", temp_adw_id, "adw_classifier", "raw_output.jsonl"
            )
            if os.path.exists(output_file):
                try:
                    ...
                    if raw_content:
                        print(f"Found raw output, attempting to parse...")
                        output_to_parse = raw_content
                    else:
                        return ADWExtractionResult()  # Empty result
                except Exception as e:
                    print(f"Failed to read raw output file: {e}")
                    return ADWExtractionResult()  # Empty result
            else:
                return ADWExtractionResult()  # Empty result
        ...
        try:
            data = parse_json(output_to_parse, dict)
...
def extract_plan_file_path(response_text: str) -> Optional[str]:
    """Extract plan file path from verbose Claude Code v2.0.30+ output.

    Claude Code v2.0.30 returns verbose explanations even when told to return
    only the file path. This function extracts the actual file path from patterns like:
    - "**File Location:** `specs/feature-xxx.md`"
    - "specs/feature-xxx.md"
    - Markdown file paths in the response

    Args:
        response_text: The full response text from the planner

    Returns:
        The extracted file path, or None if not found
    """
    import re

    # Try pattern 1: **File Location:** `path`
    match = re.search(r'\*\*File Location:\*\*\s*`([^`]+\.md)`', response_text)
    if match:
        return match.group(1)

    # Try pattern 2: Look for specs/*.md paths
    match = re.search(r'\bspecs/[^\s`\'"]+\.md\b', response_text)
    if match:
        return match.group(0)

    # Try pattern 3: Look for any .md file path at start of line or after colon
    match = re.search(r'(?:^|:\s*)([^\s]+\.md)', response_text, re.MULTILINE)
    if match:
        path = match.group(1)
        # Only return if it looks like a valid path (contains / or is in specs/)
        if '/' in path or path.startswith('specs'):
            return path
...
def find_existing_branch_for_issue(
    ...
    result = subprocess.run(
        ["git", "branch", "-a"], capture_output=True, text=True, encoding='utf-8', cwd=cwd
    )
